...
def svc_classify(x, y, search):
    ...
    for train_index, test_index in kf.split(x, y):
        ...
        accuracies.append(accuracy_score(y_test, classifier.predict(x_test))) # é usado validação cruzada dentro do conjunto de treino

        # val
        val_size = len(test_index)
        test_index = np.random.choice(train_index, val_size, replace=False).tolist() # do conjunto de treino, eu pego uma porção igual ao conjunto de teste
        train_index = [i for i in train_index if not i in test_index]
